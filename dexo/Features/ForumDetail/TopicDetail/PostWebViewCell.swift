...
protocol PostWebViewCellDelegate: AnyObject {
...
final class PostWebViewCell: UITableViewCell {
    ...
    weak var delegate: PostWebViewCellDelegate?
    ...
    func configure(
        ...
        baseURL: String,
        delegate: PostWebViewCellDelegate?,
        floorNumber: Int,
        postLink: String?
    ) {
        self.postId = post.id
    ...
    @objc private func handleTap(_ gesture: UITapGestureRecognizer) {
        ...
        for region in interactiveRegions {
            if region.frame.contains(location) {
                switch region.kind {
                case .image(let url):
                    delegate?.postWebViewCell(self, didTapImageURL: url)
                case .link(let url):
                    delegate?.postWebViewCell(self, didTapLinkURL: url)
                case .details(let index):
                    delegate?.postWebViewCell(self, didTapToggleDetails: index, postId: postId)
                }
                return
            }
        }
    ...
    @objc private func repliesButtonTapped() {
        delegate?.postWebViewCell(self, didTapShowRepliesForPostId: postId)
    }

    @objc private func replyButtonTapped() {
        guard let post = currentPost else { return }
        delegate?.postWebViewCell(self, didTapReplyToPost: post)
    }
